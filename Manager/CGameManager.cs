...
public class CGameManager : MonoBehaviour {
    ...
    public GameObject _idleMenu;
    public GameObject _soldierPanel;//업그레이드,판매용 패널
    ...
    IEnumerator BuildSoliderCoroutine()
    {
        while (true)
        {
            if (Input.GetMouseButton(0) && !_soldierPanel.activeInHierarchy)
            {
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    //int layer = (-1) - (1 << LayerMask.NameToLayer("Tile"));

                    if (layer == LayerMask.NameToLayer("Tile"))
                    {
                        bool isBuild = hit.transform.gameObject.GetComponent<CTile>().GetIsBuild();
                        //true면 이미 생성된곳,생성불가 false면 생성가능
                        if(isBuild)
                        {
                            _soldierPanel.SetActive(true);
                            _idleMenu.SetActive(false);
                            _buildMenu.SetActive(false);
                            //타일에 생성된 유닛을 가져와서 패널에 넣음
                            _soldierPanel.SendMessage("GetUnit", hit.transform.gameObject.GetComponent<CTile>()._unit);
                            Camera.main.SendMessage("GetTarget",hit.transform.gameObject.GetComponent<CTile>()._unit);
                        ...
                        else
                        {
                            ...
                            {
                                //터치한 자리에 오브젝트 생성 타일 정 가운데에 넣기위해 자식으로 들어갔다가 위치조정 후 다시 나옴
    ...
    public void OnEscapeButtonClick()
    {
        ...
        _idleMenu.SetActive(true);
        _buildMenu.SetActive(true);
    }

    //회수버튼은 Soldier 오브젝트가 필요하므로 CSoldierPanel스크립트에서 추가로 작성
